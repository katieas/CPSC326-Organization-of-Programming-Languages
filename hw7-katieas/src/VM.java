  ...
  public void run() throws MyPLException {
    ...
    while (frame != null && frame.pc < frame.instructions.size()) {
      ...
      if (instr.opcode() == OpCode.PUSH) {
        ...
        frame.variables.set(index, value);
      }
      ...
      //------------------------------------------------------------
        ...
        if (frame.operandStack.peek() instanceof Integer) {
          ...
          String x = frame.operandStack.pop().toString();
          String y = frame.operandStack.pop().toString();
          ensureNotNil(frame, x);
          ensureNotNil(frame, y);
          frame.operandStack.push(y + x);
        }
        ...
        // if (frame.operandStack.peek() instanceof Boolean || frame.operandStack.peek().equals("nil")) {
        //   Boolean x = Boolean.parseBoolean(frame.operandStack.pop().toString());
        //   Boolean y = Boolean.parseBoolean(frame.operandStack.pop().toString());
        //   ensureNotNil(frame, x);
        //   ensureNotNil(frame, y);
