  ...
  public Token nextToken() throws MyPLException {
    ...
    if (isSymbol(curr, '#')) { // check for comment
      ...
      while (isSymbol(curr, '#')) {
        ...
        while (isWhitespace(curr)) { // consume any extra whitespace
          if (isSymbol(curr, '\n')) {
            line++;
            column = 1;
          } else {
    ...
    // characters
      ...
      if (isSymbol(peek(), '\'')) { // normal char
        if (isSymbol(curr, '\n')) {
          error("found newline in character", line, column + 1);
        } else if (isSymbol(curr, '\t')) {
          error("found tab in character", line, column + 1);
        }
        tmp = "" + (char) curr;
        curr = read();
        column = column + 2;
    ...
    // strings
    else if (isSymbol(curr, '\"') || (isSymbol(curr, '\\') && isSymbol(peek(), '\"'))) {
      if (isSymbol(curr, '\\')) {
        curr = read();
      }
      curr = read();
      while (!isSymbol(curr, '\"')) {
        ...
        tmp += (char) curr;
        curr = read();
        if (isSymbol(curr, '\\') && !isSymbol(peek(), 't') && !isSymbol(peek(), 'n')) {
          curr = read();
        }
      }
      ...
      while (isDigit(peek()) || isSymbol(peek(), '.')) {
        ...
        if (isSymbol(curr, '.')) {
          if (isDouble) {
            ...
            error("missing decimal digit in double value '" + tmp + ".'", line, column);
          } else {
            isDouble = true;
          }
        }
