...
public class StaticChecker implements Visitor {
  ...
  public void visit(Program node) throws MyPLException {
    ...
    for (FunDecl fdecl : node.fdecls) {
      ...
      String funKey = fdecl.funParamKey;
      // check if function already exists
      if (symbolTable.nameExists(funKey))
        error("function '" + funKey + "' already defined", fdecl.funName);
      ...
      for (FunParam p : fdecl.params) {
        ...
        //   }
        // }
        typeInfo.add(funKey, p.paramName.lexeme(), p.paramType.lexeme());
      }

      // TODO: add each formal parameter as a component type
      // add the return type
      typeInfo.add(funKey, "return", fdecl.returnType.lexeme()); ////
    }
    ...
    if (!returnType.equals("void")) 
      error("main function must return void", new Token(null, currType, 0, 0));
  ...
  public void visit(FunDecl node) throws MyPLException {
    currType = "void";
    symbolTable.pushEnvironment();
    String returnType = typeInfo.get(node.funParamKey, "return");

    List<String> params = new ArrayList<>(typeInfo.components(node.funParamKey));
    String paramName = "";
    String paramType = "";
    for (int i = 0; i < params.size(); ++i) {
      paramName = params.get(i);
      paramType = typeInfo.get(node.funParamKey, paramName);
